...
public class QuestionGeneratorTests
{
    ...
    public async Task Eq_boost_L1_uses_four_frequencies_without_three_in_a_row()
    {
        ...
        var questions = new EqFrequencyGenerator().Generate(
            new QuestionGenerationContext(level, CatalogSeeder.QuestionCount, new Random(1)));

        questions.Count.ShouldBe(CatalogSeeder.QuestionCount);
    ...
    [Fact]
    public async Task Eq_boost_L2_uses_all_seven_frequencies_evenly()
    {
        var level = await LoadLevel("eq", "boost", 2);
        var questions = new EqFrequencyGenerator().Generate(
            new QuestionGenerationContext(level, CatalogSeeder.QuestionCount, new Random(2)));
        ...
        BalancedAnswerSequence.Counts(
            ...
            .ShouldBe([3, 3, 3, 3, 3, 3, 2], ignoreOrder: true);
    ...
    public async Task Combined_balances_boost_and_cut()
    {
        ...
        var questions = new EqFrequencyAndDirectionGenerator()
            .Generate(new QuestionGenerationContext(level, CatalogSeeder.QuestionCount, new Random(3)));

        questions.Count.ShouldBe(CatalogSeeder.QuestionCount);
        questions.Count(q => q.CorrectAnswerKey.EndsWith(":boost")).ShouldBeGreaterThan(0);
        questions.Count(q => q.CorrectAnswerKey.EndsWith(":cut")).ShouldBeGreaterThan(0);
    ...
    [Fact]
    public async Task Compression_L3_is_seven_seven_six()
    {
        var level = await LoadLevel("compression", "detection", 3);
        var questions = new CompressionChoiceGenerator()
            .Generate(new QuestionGenerationContext(level, CatalogSeeder.QuestionCount, new Random(11)));

        var counts = questions.GroupBy(q => q.CorrectAnswerKey)
            .Select(g => g.Count())
            .OrderByDescending(c => c)
            .ToArray();
        counts.ShouldBe([7, 7, 6]);
    }
